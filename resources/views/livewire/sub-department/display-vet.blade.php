    ...
    <div class="my-0 space-y-3">
        ...
        <div class="relative flex flex-col justify-start p-3 space-x-2 space-y-4 border-2">
            ...
            <div class="flex items-center sm:space-x-4">
                <div class="">
                        <div class="flex">
                            <label for="edu_department" class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2">
                                Programmes
                            </label>
                            <span class="text-red-500">*</span>
                        </div>
                    <div class="mt-1 sm:mt-0 sm:col-span-2">
                        <x-input.select wire:model="edu_department"  class="w-full" id="edu_department">
                            <div class="px-4 overflow-y-auto">
                                <option>Select Program</option>
                                @foreach (App\Models\Assets::VET as $value => $label)
                                    <option class="cursor-pointer" value={{ $value }}>{{$label}} </option>
                                @endforeach
                            </div>
                        </x-input.select>
                        {{-- <x-jet-input  type="text"  wire:model.defer="gender" class="w-full uppercase" id="gender" placeholder="gender"  />           --}}
                        <x-jet-input-error for="edu_department" class="mt-2" />
                    </div>
                </div>
        ...
        {{-- <div class="flex flex-wrap space-x-3">
            ...
            @endif hover:text-white hover:bg-blue-200  rounded-md shadow-sm cursor-pointer sm:px-3" wire:click="$set('botany', 122)"> BOTANY</div>
            <div class="px-1 pl-1  @if ($active == 123)
            bg-blue-300 text-white
            @else
            bg-gray-100 text-gray-500
            @endif hover:text-white hover:bg-blue-200 rounded-md shadow-sm cursor-pointer sm:px-3" wire:click="$set('biological', 123)"> ENVIRONMENTAL BIOLOGY</div>
            <div class="px-1 pl-1  @if ($active == 124)
            bg-blue-300 text-white
            @else
            bg-gray-100 text-gray-500
            @endif hover:text-white hover:bg-blue-200 rounded-md shadow-sm cursor-pointer sm:px-3" wire:click="$set('zoo', 124)"> ZOOLOGY</div>
        </div> --}}

        <div wire:loading wire:target=""  class="absolute z-30 space-x-4 left-1/3 animate-pulse">
