...
class DisplayPureAndApplied extends Component
{
    ...
    public function updatedChem($value){
    ...
    public function updatedInd($value){
        ...
        $this->student_courses  = Department::with(['subsidiries' => function($query){
            $query->whereLevel($this->year)->whereSemesterOffer($this->semester);
        }])->whereId($this->active)->get();
    }

    public function updatedPetch($value){
    ...
    {
        return view('livewire.sub-department.display-pure-and-applied');
    }
}
