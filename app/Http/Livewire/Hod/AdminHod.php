...
class AdminHod extends Component
{

    protected $listeners = ['updated' => '$refresh' , 'edited' => '$refresh'];
    ...
    public function fetchCom(){
        ...
        $this->depart = DepartmentUser::whereUserId(auth()->id())->first();

        $this->department = Department::whereId($this->depart->department_id)->first();

        $this->student_courses  = Department::with(['subsidiries' => function($query){

            $query->whereDepartmentId($this->depart->department_id)->whereLevel($this->edit_level)->whereSemesterOffer($this->semester)->get();
    ...
    public function render()
    {
        ...
        //$program = Program::whereDepartmentId($depart->department_id)->first();

        //dd($program->id);

        // $this->biologicalSciCourses = Course::whereProgramId($program->id)->get();

        return view('livewire.hod.admin-hod');
    }
}
